...
assert.match(app, /function businessRoleCopy\(subtype, isAdmin\)/);
assert.match(app, /function renderPerformanceWorkspace\(subtype, isAdmin\)/);
assert.match(app, /App-owned event taxonomy/);
assert.match(app, /Evidence validation ladder/);
assert.match(app, /No count loaded/);
assert.match(app, /A defined event name is not an observed result/);
assert.match(app, /Cross-tenant evidence is never aggregated in this client/);
assert.match(app, /OPERATOR OPERATIONS/);
assert.match(app, /PROVIDER OPERATIONS/);
assert.match(app, /ADMIN CONTROL/);
assert.match(app, /Demo records are illustrative and do not represent partners or approvals/);
...
assert.match(businessConsoleCss, /lc-v4-evidence-gates/);
...
assert.match(html, /app-product\.js\?v=109/);
assert.match(html, /product-shell-v2\.js\?v=16/);
...
assert.match(sw, /lc-app-investor-demo-v118/);
