...
test('membership lifecycle validates role and status and closes anonymous execution', () => {
  assert.match(migration, /next_role not in \('owner','admin','analyst','viewer'\)/i);
  assert.match(migration, /next_status not in \('active','suspended','removed'\)/i);
  assert.match(migration, /revoke all on function public\.manage_industry_org_member\(uuid, uuid, text, text\) from public, anon/i);
  assert.match(migration, /grant execute on function public\.manage_industry_org_member\(uuid, uuid, text, text\) to authenticated/i);
});
