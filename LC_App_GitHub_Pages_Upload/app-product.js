...
<div class="lc-v3-choice-wrap">${checks("languages", languages, selectedLanguages)}</div>
  ...
  function renderIndustryHome(view = state.businessView) {
    ...
    const views = {
      ...
      performance: `<section class="lc-v4-business-title"><div><span>EVIDENCE WORKSPACE</span><h1>Performance</h1><p>App-owned observed events only. CAC, retention, GGR and ROI remain To be validated.</p></div>${businessState("No commercial claims", "ready")}</section><section class="lc-v4-ops-evidence"><article><span>OBSERVED</span><h2>LC product events</h2><p>Impression, profile open, follow, Live open, handoff intent and attributable return.</p></article><article><span>OUTSIDE LC</span><h2>Operator outcomes</h2><p>Gameplay, deposits, wagering, settlement and responsible-gaming operations.</p></article></section>${isAdmin ? businessEmpty("No authorized evidence review", "Cross-tenant evidence requires explicit privileged scope.", "Protected") : renderPilotMeasurementBrief(subtype)}<section class="lc-product-card"><div class="lc-product-section-head"><h2>Attribution contract</h2><span>Observed events</span></div><div class="lc-product-flow"><span>Impression</span><span>Profile</span><span>Follow</span><span>Live</span><span>Handoff</span><span>Return</span></div></section>`,
