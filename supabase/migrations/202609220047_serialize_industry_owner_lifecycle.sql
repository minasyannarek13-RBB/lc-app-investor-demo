-- Serialize owner membership changes per organization so concurrent demotions/removals cannot leave a tenant ownerless.
begin;
...
begin
  ...
  -- Lock the tenant itself first. Every membership mutation through this RPC now
  -- serializes on one stable row before owner count is read or membership changes.
  perform 1
  from public.industry_organizations o
  where o.id = org_id
    and o.status = 'active'
  for update;
...
comment on function public.manage_industry_org_member(uuid, uuid, text, text) is
  'Owner-only tenant membership lifecycle. Serializes membership changes per tenant and prevents removal/demotion of the last active owner.';
